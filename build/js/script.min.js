...
}a||setTimeout(e(b),100)}function e(a){var b=a?"adv":"";$("."+b+"statsContent").height("auto");var c=$("."+b+"diversityContainer").height()-50,e=$("."+b+"diversityContainer").width();if(c>e){var f=e/n;f*o>c&&(f=c/o)}else{var f=c/o;f*n>e&&(f=e/n)}a?d3.select("#"+b+"diversityGraphGroup").attr("transform","scale("+f+")translate("+l.left+","+l.right+")"):d3.select("#"+b+"diversityGraphGroup").attr("transform","scale("+f+")translate("+l.left+","+l.right+")");var g=d3.select("#"+b+"diversityGraphGroup").node().getBBox().width;d3.select("#"+b+"diversityGraph").attr("height",c+l.bottom).attr("width",g*f+l.left+20),d(!0)}function f(a){console.log(a);var b=$("[name="+a+"]").is(":checked");if("rangethroughLine"===a){var c=$('[name="singletons"]').is(":checked"),d=c?["rangethroughLineYes","rangethroughLineNo"]:["rangethroughLineNo","rangethroughLineYes"];$("."+d[0]).css("display",b?"":"none"),$("."+d[1]).css("display","none")}else $("."+a).css("display",b?"":"none")}function g(){var b=$("[name=taxonLevel]").val(),c=$("[name=timeLevel]").val(),d=paleo_nav.dataUrl,e=map.getBounds(),f=e._southWest,g=e._northEast;parseInt(d3.select("#map").style("height"))<1&&(f.lng=-180,g.lng=180,f.lat=-90,g.lat=90),d+="/data1.2/occs/quickdiv.json?",d=navMap.parseURL(d),d+="&lngmin="+f.lng.toFixed(1)+"&lngmax="+g.lng.toFixed(1)+"&latmin="+f.lat.toFixed(1)+"&latmax="+g.lat.toFixed(1),d+="&count="+b+"&time_reso="+c,a(d)}function h(){var b=$("[name=taxonLevel]").val(),c=$("[name=timeLevel]").val(),d=$('[name="extant"]').is(":checked"),e=paleo_nav.dataUrl,f=map.getBounds(),g=f._southWest,h=f._northEast;parseInt(d3.select("#map").style("height"))<1&&(g.lng=-180,h.lng=180,g.lat=-90,h.lat=90),e+="/data1.2/occs/diversity.json?",e=navMap.parseURL(e),e+="&lngmin="+g.lng.toFixed(1)+"&lngmax="+h.lng.toFixed(1)+"&latmin="+g.lat.toFixed(1)+"&latmax="+h.lat.toFixed(1),e+="&count="+b+"&time_reso="+c+"&recent="+d,a(e)}function i(){var a=d3.select("#diversityGraph").attr("version",1.1).attr("xmlns","http://www.w3.org/2000/svg").node().parentNode.innerHTML,b="data:image/svg+xml;base64,"+btoa(a),c='<img src="'+b+'">';d3.select(full?"#advsvgdataurl":"#svgdataurl").html(c),j();var d=document.querySelector("canvas"),e=d.getContext("2d"),f=new Image;f.src=b,f.onload=function(){e.drawImage(f,0,0);var a=d.toDataURL("image/png"),b='<img src="'+a+'">';d3.select(full?"#advpngdataurl":"#pngdataurl").html(b);var c=document.createElement("a");c.download="diversity-curve.png",c.href=a,c.id="downloadLink",document.getElementsByTagName("body")[0].appendChild(c),c.click()}}function j(){var a=d3.select("#diversityGraph"),b=a.attr("height"),c=a.attr("width");d3.select("canvas").attr("height",b).attr("width",c)}var k,l={top:20,right:20,bottom:80,left:80},m={top:0,right:0,bottom:0,left:80},n=960,o=800-l.top-l.bottom;return d3.select(window).on("resize",d),{plot:a,resize:e,currentRequest:k,updateQuickdiv:g,updateFulldiv:h,saveImg:i,getCanvasSize:j,toggleLine:f}}();
