...
var taxaBrowser = (function(){
  ...
  // TODO: this should check both the simple and extended taxa search boxes
    ...
    if (name.length > 0) {
      ...
      d3.json(paleo_nav.dataUrl + '/data1.2/taxa/list.json?status=all&name=' + name, function(err, data) {
        if (err) {
  ...
  function getTaxonDetails(taxon) {
    ...
    d3.json(paleo_nav.dataUrl + '/data1.2/taxa/single.json?id=' + taxon.oid + '&show=attr,nav,size', function(err, data) {
      if (err) {
        paleo_nav.hideLoading();
  ...
  function getSubtaxa(taxon, rank, offset, limit) {
    ...
    if (rank > 0) {
      ...
      var url = paleo_nav.dataUrl + '/data1.2/taxa/list.json?id=' + taxon.oid + lim_str + '&show=size&rel=all_children&rank=' + rank;
